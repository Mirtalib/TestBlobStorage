...
namespace TestBlobStorage.Services
{
    public class UserService : IUserService
    {
        ...
        {
            var employee = await _context.Users.FirstOrDefaultAsync(e => e.Username == request.Username);

            if (employee is null) 
                throw new($"{request.Username} does not exist");

            if (!VerifyPassword(request.Password, employee.PasswordHash, employee.PasswordSalt)) 
        ...
        public async Task<string> UploadProfilePicture(UpdateProfilePhotoDto request)
        {
            ...
            using (var stream = file.OpenReadStream())
            {
                ...
                var result = _storageManager.UploadFile(file);
        ...
        private string CreateToken(User user)
        {
            ...
            var key = new SymmetricSecurityKey(System.Text.Encoding.UTF8.GetBytes("DehshetQirqinOlumDavaMusibetttttSecretKeydiQaqaaaaaaaaaaaaaaaaaaaaaammmmmm"));
